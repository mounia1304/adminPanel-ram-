...
const StatsDashboard = ({
  ...
  const calculateRealKPIs = () => {
    ...
    const foundTrend =
      previousWeekFound > 0
        ? `${(
            ((currentWeekFound - previousWeekFound) / previousWeekFound) *
            100
          ).toFixed(1)}%`
        : currentWeekFound > 0
        ? "+100%"
    ...
    const lostTrend =
      previousWeekLost > 0
        ? `${(
            ((currentWeekLost - previousWeekLost) / previousWeekLost) *
            100
          ).toFixed(1)}%`
        : currentWeekLost > 0
        ? "+100%"
    ...
    const matchTrend =
      previousWeekMatches > 0
        ? `${(
            ((currentWeekMatches - previousWeekMatches) / previousWeekMatches) *
            100
          ).toFixed(1)}%`
    ...
    const responseTimeData = acceptedMatches
      .map((match) => {
        const matchDate = getDateFromTimestamp(
          match.createdAt || match.timestamp
        );

        // Trouver l'objet perdu correspondant
        const lostObject = lostObjects.find(
          (obj) => obj.id === (match.lostObjectId || match.lostId)
        );
    ...
    const averageResponseTime =
      responseTimeData.length > 0
        ? (
            responseTimeData.reduce((sum, time) => sum + time, 0) /
  ...
  const generateChartData = () => {
    ...
    if (
      typeArray.length === 0 &&
      (foundObjects.length > 0 || lostObjects.length > 0)
    ) {
      const totalObjects = foundObjects.length + lostObjects.length;
      typeArray = [
        {
          name: "Téléphone",
          found: Math.ceil(foundObjects.length * 0.3),
          lost: Math.ceil(lostObjects.length * 0.25),
          total: Math.ceil(totalObjects * 0.275),
        },
        {
          name: "Sac",
          found: Math.ceil(foundObjects.length * 0.2),
          lost: Math.ceil(lostObjects.length * 0.2),
          total: Math.ceil(totalObjects * 0.2),
        },
        {
          name: "Ordinateur portable",
          found: Math.ceil(foundObjects.length * 0.15),
          lost: Math.ceil(lostObjects.length * 0.15),
          total: Math.ceil(totalObjects * 0.15),
        },
        {
          name: "Écouteurs",
          found: Math.ceil(foundObjects.length * 0.1),
          lost: Math.ceil(lostObjects.length * 0.1),
          total: Math.ceil(totalObjects * 0.1),
        },
        {
          name: "Portefeuille",
          found: Math.ceil(foundObjects.length * 0.1),
          lost: Math.ceil(lostObjects.length * 0.15),
          total: Math.ceil(totalObjects * 0.125),
        },
        {
          name: "Clés",
          found: Math.ceil(foundObjects.length * 0.08),
          lost: Math.ceil(lostObjects.length * 0.08),
          total: Math.ceil(totalObjects * 0.08),
        },
        {
          name: "Montre",
          found: Math.ceil(foundObjects.length * 0.05),
          lost: Math.ceil(lostObjects.length * 0.05),
          total: Math.ceil(totalObjects * 0.05),
        },
        {
          name: "Autre",
          found: Math.ceil(foundObjects.length * 0.02),
          lost: Math.ceil(lostObjects.length * 0.02),
          total: Math.ceil(totalObjects * 0.02),
        },
      ].filter((item) => item.total > 0);
    }
...
return (
  <div className="space-y-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          ...
          >
            ...
            <Bar
              ...
              name="Trouvés"
            />
            <Bar dataKey="lost" stackId="a" fill={COLORS.lost} name="Perdus" />
          </BarChart>
        </ResponsiveContainer>
      </ChartCard>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {/* Lieux de découverte */}
      <ChartCard title="Lieux de découverte" subtitle="Top 8 des emplacements">
        <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {realKPIs.averageResponseTime || 0} jours
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Satisfaction:</span>
              <span className="font-medium text-green-600">
                {realKPIs.satisfaction || 0}%
              </span>
            </div>
          </div>
        </div>
